...
class ConditionalLinear(nn.Linear):
    ...
    def __init__(self, input_size, n_outputs, cond_label_size=None):
...
class AE(torch.nn.Module):
    ...
    def __init__(self, input_dim, dim=128, cond_label_size=None):
        # print(self)
        # matplotlib.scatter(self)
        print("first leakyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy")
        super().__init__()
        self.enc1 = ConditionalLinear(input_dim, dim, cond_label_size)
        self.enc1_bn = nn.BatchNorm1d(dim)
        # self.enc1_bn = nn.GroupNorm(num_channels=10, num_groups=2)
        self.enc2 = ConditionalLinear(dim, dim, cond_label_size)
        self.enc2_bn = nn.BatchNorm1d(dim)
        self.enc3 = ConditionalLinear(dim, dim, cond_label_size)
        self.enc3_bn = nn.BatchNorm1d(dim)
        self.enc4 = ConditionalLinear(dim, dim, cond_label_size)
        self.enc4_bn = nn.BatchNorm1d(dim)
        self.enc5 = ConditionalLinear(dim, 8, cond_label_size)
        self.enc5_bn = nn.BatchNorm1d(8)

        self.dec1 = ConditionalLinear(8, dim, cond_label_size)
        self.dec1_bn = nn.BatchNorm1d(dim)
        self.dec2 = ConditionalLinear(dim, dim, cond_label_size)
        self.dec2_bn = nn.BatchNorm1d(dim)
        self.dec3 = ConditionalLinear(dim, dim, cond_label_size)
        self.dec3_bn = nn.BatchNorm1d(dim)
        self.dec4 = ConditionalLinear(dim, dim, cond_label_size)
        self.dec4_bn = nn.BatchNorm1d(dim)
        self.dec5 = ConditionalLinear(dim, dim, cond_label_size)
        self.dec5_bn = nn.BatchNorm1d(dim)

        self.out = ConditionalLinear(dim, input_dim, cond_label_size)

    def forward(self, x, y=None):
        # print(x.shape)
        # print("bob leakyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy ")

        x = F.leaky_relu(self.enc1_bn(self.enc1(x, y)))
        x = F.leaky_relu(self.enc2_bn(self.enc2(x, y)))
        x = F.leaky_relu(self.enc3_bn(self.enc3(x, y)))
        x = F.leaky_relu(self.enc4_bn(self.enc4(x, y)))
        x = F.leaky_relu(self.enc5_bn(self.enc5(x, y)))
        # print("bob second leakyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy")
        x = F.leaky_relu(self.dec1_bn(self.dec1(x, y)))
        x = F.leaky_relu(self.dec2_bn(self.dec2(x, y)))
        x = F.leaky_relu(self.dec3_bn(self.dec3(x, y)))
        x = F.leaky_relu(self.dec4_bn(self.dec4(x, y)))
        x = F.leaky_relu(self.dec5_bn(self.dec5(x, y)))
